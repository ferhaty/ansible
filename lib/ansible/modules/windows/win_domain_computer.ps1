...
Function Set-ConstructedState($initial_state, $desired_state) {
  ...
  If ($initial_state.distinguished_name -cne $desired_state.distinguished_name) {
    ...
    Try {
      ...
      Fail-Json -obj $result -message "HashTable $($desired_state.distinguished_name.count) - $($desired_state.distinguished_name.keys[0]) - $($desired_state.distinguished_name.values[0]) - Initial state DN $($initial_state.distinguished_name) does not match desired state DN $($desired_state.distinguished_name). Failed to move the AD object $($desired_state.name) to $($desired_state.ou) OU: $($_.Exception.Message)"
    }
  }
  $result.changed = $true
}
